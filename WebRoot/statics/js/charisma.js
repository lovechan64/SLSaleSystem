...
//    $('#reply_createTime').val((d.getMonth()+1)+'/'+d.getDate()+'/'+d.getFullYear());
	...
	for(var i = 0;i<json.length;i++){
		...
		//config main menu
		result = result + '<li class="nav-header hidden-tablet" onclick="clickMenu('+i+","+json.length+');" style="cursor:pointer;">'+json[i].mainMenu.functionName+'</li>';
		//config sub menus
		var style = "display: none;";
		if(i==targetId){
			style = "display: block;";
		}	
		result = result + "<li><ul class=\"nav nav-tabs nav-stacked\" id=\"test"+i+"\" style='"+style+"'>";
		for(var j=0;j<json[i].subMenus.length;j++){
			...
			result = result + "<li><a class=\"ajax-link\" style=\"cursor:pointer;\"  href='"+json[i].subMenus[j].funcUrl+"?targetId="+i+"'><i class="+pic+"></i><span class=\"hidden-tablet\">"+json[i].subMenus[j].functionName + "</span></a></li>";
		}
		result = result +"</ul></li>";
	}
	$("#menus").append(result);
...
function clickMenu(i,menulength){
	//alert(i+"      ======== "+menulength);
	$("#test"+i).toggle(500);
